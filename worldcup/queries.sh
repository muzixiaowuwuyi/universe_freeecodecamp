...
echo "$($PSQL "with results AS ((select teams.name from teams full join games on teams.team_id = games.winner_id where games.round = 'Eighth-Final' and games.year=2014)    union   (select teams.name from teams full join games on teams.team_id = games.opponent_id where games.round = 'Eighth-Final' and games.year=2014))SELECT name FROM results ORDER BY name ASC  ")"
